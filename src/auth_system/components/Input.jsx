...
const Input = ({ label, type = 'text', value, onChange, placeholder }) => {
  return (
    <div className="flex flex-col">
      {label && <label className="text-grayText dark:text-grayText mb-1">{label}</label>}
      <input
        type={type}
        value={value}
        onChange={onChange}
              ...
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div className="bg-lightCard dark:bg-card p-6 rounded-lg shadow-lg w-full max-w-md">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setIsEditing(false)} className="text-grayText dark:text-grayText hover:text-red-500">
                      ✕
                    </button>
                  </div>
